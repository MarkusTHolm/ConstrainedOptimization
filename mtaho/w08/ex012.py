...
def randomLP(n, alpha, density, seed):
    ...
    H = np.sqrt(eps)*sp.sparse.eye(n, n)
...
density = 1
seed = 100
H, g, A, bl, bu, l, u = randomLP(n, alpha, density, seed)
x0 = np.zeros((n, 1))
...
x0 = 0.5*(np.max(bl) + np.min(bu))*np.ones((n, 1))
sol = Solvers.LPSolverInteriorPoint(g, A.T, b, x0)